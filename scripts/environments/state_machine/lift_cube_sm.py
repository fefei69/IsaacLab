...
def main():
    ...
    # # parse configuration
    env_cfg: LiftEnvCfg = parse_env_cfg(
        "Isaac-Lift-Cube-Wxai-IK-Abs-v0",
        device=args_cli.device,
        num_envs=args_cli.num_envs,
        use_fabric=not args_cli.disable_fabric,
    ) # type: ignore
    # create environment
    env = gym.make("Isaac-Lift-Cube-Wxai-IK-Abs-v0", cfg=env_cfg)
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            goal_marker.visualize(actions[:, 0:3] + env.unwrapped.scene.env_origins, actions[:, 3:7])
            # reset state machine
            if dones.any():
                print(f"Resetting {dones.sum().item()} environments.")
