...
def main():
    ...
    # create environment
    env = gym.make("Isaac-Lift-Cube-Wxai-v0", cfg=env_cfg)  # type: ignore
    ...
    joint_action = torch.zeros((env.unwrapped.num_envs, 7), device=env.unwrapped.device)
    while simulation_app.is_running():
        # run everything in inference mode
        with torch.inference_mode():
            # step environment
            dones = env.step(joint_action)[-2]
            ...
            # actions = actions.clone()[: , :-1]

            ik_commands[:] = actions[:, :7]
            # reset controller
            diff_ik_controller.reset()
            ...
            joint_action = torch.concat([joint_pos_des, 0.044 * torch.ones(env.unwrapped.num_envs, 1, device=env.unwrapped.device)], dim=-1)
