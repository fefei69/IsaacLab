...
WXAI_CFG = ArticulationCfg(
    ...
    actuators={"first4_joints": ImplicitActuatorCfg(
                                joint_names_expr=["joint_[0-3]"], 
                                effort_limit_sim=27.0, 
                                stiffness=800.0, # 10000
                                damping=40.0, # 100.0
...
# WXAI_CFG.actuators["joint4"].stiffness = 100000.0   
# WXAI_CFG.actuators["joint4"].damping = 1000.0
...
class TableTopSceneCfg(InteractiveSceneCfg):
    ...
    # articulation
    robot = WXAI_TEST_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        if count % 150 == 0:
            ...
            joint_pos_des = diff_ik_controller.compute(ee_pos_b, ee_quat_b, jacobian, joint_pos) # n, 6

        print("current_goal_idx:", current_goal_idx )
        print("joint pos des:", joint_pos_des[0].cpu().numpy() )
        ...
        # apply actions
        robot.set_joint_position_target(joint_pos_des, joint_ids=robot_entity_cfg.joint_ids) # joint 0 to 5
