...
class M3CabinetEnv(DirectRLEnv):
    ...
    def _get_dones(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Robot fell
        fell = self.robot.data.root_pos_w[:, 2] < -0.01
    ...
    def _debug_vis_callback(self, event):
        ...
        # Drawer handle frame
        handle_body_ids = self.cabinet.find_bodies("drawer_handle_top")[0]  # adjust name to match your URDF/USD
        drawer_handle_pos = self.cabinet.data.body_pos_w[:, handle_body_ids[0]]
        drawer_handle_quat = self.cabinet.data.body_quat_w[:, handle_body_ids[0]]
        drawer_handle_pos += torch.tensor([0.305, 0.0, 0.01], device=self.device)  # Offset to handle position
        self.drawer_goal_frame_visualizer.visualize(drawer_handle_pos, drawer_handle_quat)
